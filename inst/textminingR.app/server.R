...
server <- shinyServer(function(input, output, session) {
    ...
    tokens_dict <- eventReactive(input$dictionary, {
        ...
        dictionary_list_1 <- textminingR::dictionary_list_1
        dictionary_list_2 <- textminingR::dictionary_list_2
    ...
    stopwords_list <- textminingR::stopwords_list
    ...
    output$step3_plot <- plotly::renderPlotly({
        dfm_init() %>% textminingR::plot_word_frequency(n = 20)
    })
    ...
    observeEvent(input$remove,{
        if(!is.null(input$remove.var)){
            output$step4_plot <- plotly::renderPlotly({
                dfm_outcome() %>% textminingR::plot_word_frequency(n = 20)
            })
        }
    })
    ...
    K_search <- eventReactive(
        eventExpr = input$search, {
            ...
            if (!is.null(input$categorical_var) && !is.null(input$continuous_var)) {
            ...
            } else if (!is.null(input$categorical_var) && is.null(input$continuous_var)) {
                ...
                stm::searchK(
                    ...
                    init.type = "Spectral",
                    K = print_K_range_1(),
                    prevalence =~ stm::s(eval(parse(text = input$continuous_var))),
                    verbose = TRUE)
    ...
    stm_K_number <- eventReactive(
        eventExpr = input$run, {
            
            utils::memory.limit(24000)
            set.seed(01234)
            
            if (!is.null(input$categorical_var_2) && !is.null(input$continuous_var_2)) {
            ...
            } else if (!is.null(input$categorical_var_2) && is.null(input$continuous_var_2)) {
                ...
                stm::stm(
                    ...
                    init.type = "Spectral",
                    K = print_K_number(),
                    prevalence =~ stm::s(eval(parse(text = input$continuous_var_2))),
                    verbose = TRUE)
    ...
    output$topic_term_plot <- renderPlot({
        ...
        beta_td() %>% textminingR::plot_topic_term(topic_names = tn,
    ...
    top_terms_selected <- reactive({
        beta_td() %>% textminingR::examine_top_terms(top_n = input$top_term_number_2)
    })
    ...
    gamma_terms <- reactive({
        ...
        dd = gamma_td %>% textminingR::examine_gamma_terms(top_terms = top_terms_selected())
    ...
    observeEvent(input$display,{
        ...
        output$topic_by_prevalence_plot2 <- plotly::renderPlotly({
            gamma_terms() %>%
                textminingR::plot_topic_probability(topic_names = tn,
    ...
    observeEvent(input$display,{
        ...
        output$topic_by_prevalence_table <- DT::renderDataTable({
            gamma_terms() %>%
                textminingR::plot_topic_probability_table(topic_names = tn,
                                             top_n = input$topic_number) %>%
    ...
    observe({
        updateSelectizeInput(session, "topic_texts",
    ...
    effect_stm_K_number <- eventReactive(
        eventExpr = input$effect, {
    ...
    observeEvent(input$display_cat,{
        ...
        if(!is.null(input$effect_by_cat_btn)) {
    ...
    observe({
        updateSelectizeInput(session, "effect_by_con_btn",
    ...
    observeEvent(input$display_con, {
        ...
        if(!is.null(input$effect_by_con_btn)) {
            ...
            output$by_con_plot <- renderPlot({
                ...
                effects_continuous_var() %>%
                    ...
                    facet_wrap(~ topic, ncol = 3, scales = "free") +
                    scale_y_continuous(labels = numform::ff_num(zero = 0, digits = 3)) +
                    geom_line(size = 0.5, color = "#337ab7") +
                    xlab("") +
                    ylab("Topic proportion") +
                    theme_minimal(base_size = 14) +
                    theme(
    ...
    observeEvent(input$plot_dendrogram, {
        ...
        output$dendro_plot <- plotly::renderPlotly({
            ggdendro::ggdendrogram(hclust(), rotate = TRUE) +
                ...
                theme(
                    axis.line = element_line(size = 0.1, color = "#3B3B3B"),
        ...
        output$word_network_plot <- renderPlot({
            ...
            word_network <- term_cor %>%
                ...
                geom_edge_link(aes(edge_alpha = correlation,
                                   edge_width = correlation), edge_colour = "#47a0ed")+
    ...
    observe({
        updateSelectizeInput(session, "continuous_var_3",
                          choices = colnames_con_2(),
                          selected = "")
    })
